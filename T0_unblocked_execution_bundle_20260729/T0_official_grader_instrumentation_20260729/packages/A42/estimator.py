"""Kerdock 5-design with weight-preencoded row-streamed Winograd propagation."""
...
from fast_matmul import prepare_right_p3_d5, winograd_hybrid_p3_d5_prepared_right
...
class Estimator(BaseEstimator):
    ...
    def predict(self, mlp: MLP, budget: int) -> fnp.ndarray:
        ...
        for start in range(0, _TOTAL_ROWS, _STREAM_ROWS):
            activation = first_activation[start : start + _STREAM_ROWS]
            for prepared in prepared_weights:
                activation = fnp.maximum(
                    winograd_hybrid_p3_d5_prepared_right(activation, prepared),
                    0.0,
                )
            chunk_sum = fnp.sum(activation.astype(fnp.float64), axis=0)
