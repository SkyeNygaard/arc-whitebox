...
| I4 | Haar-random relative orientation makes every orientation-blind correction useless | Analytically proved | Independent Haar orientation and mass-one cubature |
...
Consequently, if $g$ is Haar and independent of all orientation-blind runtime information $\mathcal G\subseteq\sigma(f)$, then
...
Summing with $\sum_iw_i=1$ proves the first identity. Conditional mean zero follows because $g$ is independent of $\mathcal G$, and Theorem 4.1 gives zero correction value. $\square$
...
Both kernels are positive semidefinite. Under $K_\infty$, rule zero has risk zero and rule one has risk $g$. Under $K_m$, their risks are $\varepsilon$ and $g$. For $\varepsilon>g$, the optimizer reverses. Since $g$ can be arbitrarily small, optimizer identity is not structurally stable without a quantitative gap-versus-perturbation comparison.
...
4. equal-loading common-bias information, independent Haar-orientation-blind information, and feature classes satisfying a phase-flip symmetry have zero absolute-phase correction value;
