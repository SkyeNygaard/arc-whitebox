...
int main(int argc,char**argv){if(argc!=6){fprintf(stderr,"usage: %s hpp.tsv input.tsv mode(regen|verify) output.json log.txt\n",argv[0]);return 2;}mpfr_set_default_prec(PREC);ii(&PI);mpfr_const_pi(PI.lo,MPFR_RNDD);mpfr_const_pi(PI.hi,MPFR_RNDU);load_hpp(argv[1]);FILE*f=fopen(argv[2],"r");FILE*log=fopen(argv[5],"w");if(!f||!log){perror("open");return 2;}char line[MAX_LINE];long initial=0;Stats S;stats_init(&S);
 while(fgets(line,sizeof line,f)){char*save=NULL;char*tok=strtok_r(line,"\t\r\n",&save);if(!tok)continue;long idx=atol(tok);char*a=strtok_r(NULL,"\t\r\n",&save);char*b=strtok_r(NULL,"\t\r\n",&save);char*sg=strtok_r(NULL,"\t\r\n",&save);if(!a||!b||!sg){fprintf(stderr,"bad line\n");return 2;}mpq_t aq,bq;mpq_init(aq);mpq_init(bq);if(!parse_q(aq,a)||!parse_q(bq,b)){fprintf(stderr,"bad q\n");return 2;}int sign=strcmp(sg,"negative")==0?-1:1;initial++;
  if(strcmp(argv[3],"regen")==0)recurse(aq,bq,sign,0,&S);else {I R;ii(&R);gpp_range(&R,aq,bq);int good=sign<0?mpfr_sgn(R.hi)<0:mpfr_sgn(R.lo)>0;if(!good){fprintf(stderr,"verify fail idx=%ld\n",idx);return 6;}S.accepted++;mpfr_t m;mpfr_init2(m,PREC);if(sign<0)mpfr_neg(m,R.hi,MPFR_RNDD);else mpfr_set(m,R.lo,MPFR_RNDD);if(mpfr_cmp(m,S.minmargin)<0)mpfr_set(S.minmargin,m,MPFR_RNDD);mpfr_clear(m);ci(&R);}if(initial%25==0){fprintf(log,"%ld\n",initial);fflush(log);}mpq_clear(aq);mpq_clear(bq);
 }
 fclose(f);fclose(log);write_json(argv[4],argv[3],initial,S.accepted,S.split,S.maxdepth,S.minmargin,1);printf("passed mode=%s initial=%ld accepted=%ld split=%ld maxdepth=%d\n",argv[3],initial,S.accepted,S.split,S.maxdepth);for(int i=0;i<4;i++){ci(&HPPI[i]);mpq_clear(HPPQ[i]);}ci(&PI);mpfr_clear(S.minmargin);return 0;}
