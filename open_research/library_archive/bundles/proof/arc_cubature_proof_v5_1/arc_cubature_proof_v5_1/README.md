...
- `PROOF_MANIFEST.sha256` — fixed verification hashes for every canonical source, artifact, and generated curvature chunk. Archive authenticity requires the separately published archive digest; the manifest alone is not an external timestamp.
...
- CPython 3.11, 3.12, or 3.13 (3.13.5 reproduced in the current audit)
- `mpmath==1.3.0` only for independent implementation audits; the formal directed
