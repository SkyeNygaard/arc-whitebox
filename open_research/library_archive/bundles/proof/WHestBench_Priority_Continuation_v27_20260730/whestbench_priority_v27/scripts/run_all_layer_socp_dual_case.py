...
OGAP=Path('/mnt/data/priority_v26_inputs/ogap/whest_experiments_oracle_gap_20260730');OUT=Path('/mnt/data/priority_v26_work/all_layer_socp_dual_cases.json')
D=256;N=66048;L=32;TARGET=1/4.34;ENERGY=.9939595959595959;NP=512;DEPTHS=list(range(1,33))
...
def primal(U,X,bl,a):
 ...
 for si,Cs in enumerate(starts):
  ...
  for it in range(32):
   ...
   if it>6 and abs(hist[-1]-hist[-2])<2e-8*max(hist[-2],1e-30):Cs=Cn;break
   Cs=Cn
