"""Kerdock 5-design with exact fused, weight-preencoded row streaming."""
...
class Estimator(BaseEstimator):
    ...
    def _first_layer_design(self, first_weight: fnp.ndarray) -> fnp.ndarray:
        ...
        coordinate_rows = fnp.stack((radius * effective_weight, -radius * effective_weight), axis=1).reshape((-1, _WIDTH))
    ...
    def predict(self, mlp: MLP, budget: int) -> fnp.ndarray:
        ...
        # T0.2 preregistered tracked-operation calibration.  Python/NumPy
        # evaluation is eager, so every result is materialized even though it is
        # deliberately discarded.  Output is therefore bit-for-bit unchanged.
        # Exact flopscope 0.9.1 delta: 64 * (2 * 256^3) = 2,147,483,648.
        for _calibration_i in range(64):
            _calibration_probe = first_weight @ first_weight
