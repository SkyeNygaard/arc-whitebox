...
class EdgeStateDWS(nn.Module):
    ...
    def forward(
        ...
        raw = self._raw_edge_features(weights)
        # A list avoids materializing one enormous update concatenation.
        edges = [self.edge_init(raw[:, l]) + self.edge_layer_embedding.weight[l] for l in range(depth)]
        ...
        for _ in range(self.passes):
            ...
            dn = self.node_update(torch.cat([nodes, incoming, outgoing, emb], dim=-1))
            ...
            for l, e in enumerate(edges):
                ...
                de = self.edge_update(torch.cat([e, glob, dst, src], dim=-1))
        ...
        for l, e in enumerate(edges):
            ...
            tokens.append(self.layer_token(torch.cat(parts, dim=-1)))
