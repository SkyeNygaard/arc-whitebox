...
def gaussian_source_cov(base_cov: np.ndarray, mu: np.ndarray, var: np.ndarray, actual_post_cov: np.ndarray) -> np.ndarray:
    """Local Gaussian-closure source for a pilot, used only for confidence."""
    _, post_var, gate = relu_gaussian(mu, var)
    pred = base_cov * np.outer(gate, gate)
    np.fill_diagonal(pred, post_var)
    return 0.5 * ((pred - actual_post_cov) + (pred - actual_post_cov).T)
...
def run_one(network_id: int, xk_np: np.ndarray, truth_n: int, chunk: int) -> dict[str, Any]:
    ...
    with torch.no_grad():
        for layer, wt in enumerate(ws):
            ...
            for gi, rows in enumerate(row_groups):
                pc = w.T @ pilot_post_covs[gi] @ w
                pc = 0.5 * (pc + pc.T)
                _, _, qc = raw_moments(post, rows, False)
                pre_covs.append(pc); post_covs.append(qc)
                sources.append(gaussian_source_cov(pc, pre_mean, pre_var, qc))
            pre_cov = 0.5 * (pre_covs[0] + pre_covs[1])
            post_cov = 0.5 * (post_covs[0] + post_covs[1])
            delta_mu, delta_cov, tr = update_defect(
                delta_mu, delta_cov, w,
                pre_mean, pre_var, post_mean, post_var,
                pre_cov, post_cov,
            )
            tr['layer'] = layer
            tr['source_disagreement'] = float(np.linalg.norm(sources[0] - sources[1]) / max(np.linalg.norm(0.5 * (sources[0] + sources[1])), 1e-30))
            trace.append(tr)
            pilot_post_covs = post_covs
            x = post
            if layer == TARGET:
                target_h = x.clone()
                target_state = (delta_mu.copy(), delta_cov.copy())
                target_disagreement = float(np.mean([z['source_disagreement'] for z in trace[-8:]]))
    ...
    fit = fr.fit_crossfit(X, x.double().numpy())
    ...
    candidate_lower = lower_anchor_selected(m, mu, diag_second, rowdir_second, indices, directions)

    ref1 = fr.stream_reference(ws, truth_n, 41_000_000 + 2 * network_id, chunk)
    ref2 = fr.stream_reference(ws, truth_n, 41_000_001 + 2 * network_id, chunk)
    y1, y2 = ref1['y'], ref2['y']
    ...
    pred_grid = [fr.estimate_from_fit(fit, sample_anchor + a * candidate_lower)[0] for a in alpha_grid]
    oracle_pred, _ = fr.estimate_from_fit(fit, sample_anchor + oracle_lower)
    def mse(pred): return float(np.mean((pred - 0.5 * (y1 + y2)) ** 2))
    def unbiased(pred): return float(np.mean((pred - y1) * (pred - y2)))
    base_mse = mse(base_pred)
    return {
        'network_id': network_id, 'weight_seed': weight_seed, 'weight_sha256': weight_hash,
        'truth_n_per_half': truth_n, 'baseline_mse': base_mse, 'baseline_unbiased_mse': unbiased(base_pred),
        'alpha_grid': alpha_grid.tolist(), 'candidate_mse_grid': [mse(p) for p in pred_grid],
        'candidate_unbiased_mse_grid': [unbiased(p) for p in pred_grid],
        'oracle_lower_mse': mse(oracle_pred), 'oracle_lower_unbiased_mse': unbiased(oracle_pred),
        'anchor': {
            'candidate_relative_error': float(np.linalg.norm(candidate_lower - oracle_lower) / max(np.linalg.norm(oracle_lower), 1e-30)),
            'candidate_cosine': float(np.dot(candidate_lower, oracle_lower) / max(np.linalg.norm(candidate_lower) * np.linalg.norm(oracle_lower), 1e-30)),
            'pilot_disagreement': target_disagreement,
            'candidate_norm': float(np.linalg.norm(candidate_lower)), 'oracle_norm': float(np.linalg.norm(oracle_lower)),
        },
        'target_delta': {'pilot_mean_norms': [float(np.linalg.norm(dm))], 'pilot_cov_fro': [float(np.linalg.norm(dc))]},
        'traces': [trace], 'runtime_seconds': float(time.perf_counter() - t0),
    }
