...
# prederived primary statistic: corr_zw (dimensionless signed complete-basis flux)
j=5
...
out={'feature_correlations':cors,'primary':{'statistic':'corr_zw = Corr_b((X_b-q)·template, (Y_b-Ybar)·normalized(template·beta))','linear_coef':float(model.coef_[0]),'intercept':float(model.intercept_),'oof_replay_shrink':lam,'validation':metrics(va,pv)},'incremental_ops_estimate':129*(32+256+8)}
(OUT/'ADJOINT_BASIS_FLUX_DEVELOPMENT.json').write_text(json.dumps(out,indent=2));print(json.dumps(out,indent=2))
