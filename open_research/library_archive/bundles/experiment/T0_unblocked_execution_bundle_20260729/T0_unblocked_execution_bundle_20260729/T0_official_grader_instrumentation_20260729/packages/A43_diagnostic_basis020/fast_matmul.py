"""Tracked depth-5 Winograd with cached right forms and fused layer transitions."""
from __future__ import annotations
import flopscope.numpy as fnp

PreparedRight = tuple[tuple[fnp.ndarray, ...], ...]
...
def prepare_right_p3_d5(weight: fnp.ndarray) -> PreparedRight:
    ...
    return tuple(_right_forms(part) for part in level_four)
...
def _assemble_tree(tree: tuple[tuple[fnp.ndarray, ...], ...]) -> fnp.ndarray:
    return fnp.block(
        [
...
def _decode(products: fnp.ndarray) -> fnp.ndarray:
    q11, q12, q21, q22 = _decode_quadrants(products)
    return fnp.block([[q11, q12], [q21, q22]])

def _decode_quadrants(
    products: fnp.ndarray,
) -> tuple[fnp.ndarray, fnp.ndarray, fnp.ndarray, fnp.ndarray]:
    p1 = products[..., 0, :, :]
    p2 = products[..., 1, :, :]
    p3 = products[..., 2, :, :]
...
def _to_final_quadrants(
    left: fnp.ndarray,
    prepared: PreparedRight,
    already_encoded_once: bool,
) -> tuple[fnp.ndarray, fnp.ndarray, fnp.ndarray, fnp.ndarray]:
    encoded_left = left
    levels = 2 if already_encoded_once else 3
    for _ in range(levels):
        encoded_left = _encode_left_once(encoded_left)
    products = _assemble_tree(_depth_two_output_tree_prepared(encoded_left, prepared))
    for _ in range(2):
        products = _decode(products)
    return _decode_quadrants(products)

def _relu_quadrants_to_first_encoding(
    quadrants: tuple[fnp.ndarray, fnp.ndarray, fnp.ndarray, fnp.ndarray],
) -> fnp.ndarray:
    a11 = fnp.maximum(quadrants[0], 0.0)
    a12 = fnp.maximum(quadrants[1], 0.0)
    a21 = fnp.maximum(quadrants[2], 0.0)
    a22 = fnp.maximum(quadrants[3], 0.0)
    s1 = a21 + a22
    s2 = s1 - a11
    s3 = a11 - a21
    s4 = a12 - s2
...
def first_layer_chunk_to_relu_encoding(
    left: fnp.ndarray,
    prepared: PreparedRight,
) -> fnp.ndarray:
    return _relu_quadrants_to_first_encoding(
        _to_final_quadrants(left, prepared, False)
    )

def encoded_chunk_to_relu_encoding(
    encoded_once: fnp.ndarray,
    prepared: PreparedRight,
) -> fnp.ndarray:
    return _relu_quadrants_to_first_encoding(
        _to_final_quadrants(encoded_once, prepared, True)
    )

def encoded_chunk_to_final_sum(
    encoded_once: fnp.ndarray,
    prepared: PreparedRight,
) -> fnp.ndarray:
    q11, q12, q21, q22 = _to_final_quadrants(encoded_once, prepared, True)
    a11 = fnp.maximum(q11, 0.0)
    a12 = fnp.maximum(q12, 0.0)
    a21 = fnp.maximum(q21, 0.0)
    a22 = fnp.maximum(q22, 0.0)
    left_sum = fnp.sum(a11.astype(fnp.float64), axis=0) + fnp.sum(a21.astype(fnp.float64), axis=0)
    right_sum = fnp.sum(a12.astype(fnp.float64), axis=0) + fnp.sum(a22.astype(fnp.float64), axis=0)
    return fnp.concatenate((left_sum, right_sum), axis=0)
