...
with contextlib.redirect_stdout(io.StringIO()):ns=runpy.run_path('/mnt/data/explore_markov012345.py')
C=ns['C'];decI=ns['decI'];R=ns['R'];G=ns['G'];mon=ns['mon']
for k in range(5):
 r=json.load(open(f'/mnt/data/tensor6_degree_{k}.json'));C[(6,k)]=R(decI(r['lo'],r['hi'])/c.M)
for k in [5,6]:
 r=json.load(open(f'/mnt/data/tensor6_degree_{k}_s2.json'));C[(6,k)]=R(decI(r['lo'],r['hi'])/c.M)
for k in range(7,17):C[(6,k)]=c.I.point(0)
states=list(range(7));v=[c.I.point(0),c.I.point(1)]+[c.I.point(0)]*5
