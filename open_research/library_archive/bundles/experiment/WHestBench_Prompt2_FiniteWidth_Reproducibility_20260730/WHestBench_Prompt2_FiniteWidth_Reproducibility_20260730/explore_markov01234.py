# Build on 0123 namespace and add tensor4 row low degrees 0..4.
from fractions import Fraction
import json,runpy,contextlib,io
import prompt2_full_hermite_core as c
with contextlib.redirect_stdout(io.StringIO()): ns=runpy.run_path('/mnt/data/explore_markov0123.py')
C=ns['C'];decI=ns['decI'];R=ns['R'];G=ns['G'];mon=ns['mon']
for k in range(5):
 rr=json.load(open(f'/mnt/data/tensor4_degree_{k}.json'));C[(4,k)]=R(decI(rr['lo'],rr['hi'])/c.M)
for k in range(5,17):C[(4,k)]=c.I.point(0)
states=list(range(5));v=[c.I.point(0),c.I.point(1),c.I.point(0),c.I.point(0),c.I.point(0)]
...
print('ratios',[(n,float(r)) for r,n in rat[:8]],'floor',float(F*rat[0][0]))
