...
5. **Arbitrary-signed global floor.** A coefficientwise combination of squared reproducing kernels and exact rank/trace obstructions proves that every static mass-one signed rule with at most 66,048 arbitrary nodes retains at least 32.468% of Kerdock MSE.
6. **Scope boundary.** Constructive counterexamples show why none of these results implies signed near-optimality, adaptive, nonlinear or transformed-residual optimality.

## 2. Scope matrix

| Result | Width | Nodes/support | Weights | Adaptation | Estimator | Status |
|---|---|---|---|---|---|---|
| Arbitrary-node near-optimality | Infinite | Any sphere nodes, <=66,048 | Nonnegative, mass one | Network-independent only | Linear | Computer-assisted certified |
| All-degree auxiliary LP | Kernel certificate | Gegenbauer expansion | Nonnegative nonconstant coefficients | None | Lower certificate | Computer-assisted certified |
| Kerdock-line allocation | Infinite and finite | Fixed 33,024 symmetrized lines | Arbitrary real, mass one | Static | Linear | Proved under explicit model |
| Complete-support uniform weights | Any width | Complete fixed Kerdock support | Fixed mass-one | None | Linear | Proved |
| Arbitrary signed-node floor | Infinite kernel | Any sphere nodes, <=66,048 | Arbitrary real, mass one | Static | Linear | Computer-assisted certified; >=32.468% of Kerdock MSE |
| Arbitrary-node finite-width T22 | Finite | Any nodes | Even nonnegative case unresolved | Static | Linear | Open |
| Adaptive/nonlinear methods | Any | Any | Any | Network dependent | Nonlinear allowed | Open |
...
## 9. Arbitrary signed-node global floor

For any finite harmonic degree set `S` containing degree zero, let `H_S` be the corresponding direct-sum feature space, of dimension `D_S`, and let `L_S` be its reproducing kernel. For a static signed mass-one rule with at most `N` nodes, the feature moment matrix has rank at most `N` and trace `D_S`, so

\[
R_{L_S^2}(Q)=\|I-M_S(Q)\|_F^2
\ge {D_S^2\over N}-D_S.
\]

The released certificate combines eleven such squared reproducing kernels with positive rational multipliers. Directed interval lower bounds for the first 26 nonconstant Gegenbauer coefficients of `K_32`, together with exact rational product coefficients, prove coefficientwise domination. Therefore every static arbitrary-node real-weight mass-one rule with at most 66,048 nodes satisfies

\[
R_{K_{32}}(Q)\ge 7.90161513053615965\times10^{-8}
\ge0.3246802745520963\,R_{Kerdock}.
\]

Thus arbitrary signed nodes can improve over Kerdock by at most approximately `3.079953x` under this theorem. This is a substantial global closure of the off-support signed loophole, but it is not signed near-optimality and does not cover nonlinear or network-adaptive estimators.

## 10. Scope guards and counterexamples
...
5. **Off-support signed weights.** The multi-rank theorem prevents arbitrary signed rules from reducing the limiting-kernel MSE below 32.468% of Kerdock, but it still permits a factor of roughly 3.08 and therefore does not establish signed near-optimality.
...
T22 uses exact rational coefficients, directed interval arithmetic, deterministic interval chunks, exact multiplicities, a directed kernel-mean enclosure and a hash manifest. A complete clean regeneration rebuilt all 1,421 certified subintervals and passed the 59-file manifest. The full theorem-critical path also has an implementation-diverse direct-C GMP/MPFR reproduction. T16 adds exact integer recurrences, interval sixth-derivative bounds and a Krawczyk coefficient enclosure. Its primal numerical certificate now reproduces in both Decimal/libmpdec and an independent `mpmath.iv` implementation, while the reduced-cost recurrence has a separate C++ exact-integer audit. The results are computer-assisted, not proof-assistant formalized.

The proof release must include:
...
- The T16 numerical primal certificate has two internal interval-stack implementations, but named external human mathematical review remains required.
